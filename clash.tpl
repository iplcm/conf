...
## 感谢：https://github.com/ConnersHua/Profiles
## 链接：{$userapiUrl}
#---------------------------------------------------#
...
log-level: info #{if array_key_exists("log-level",$opts)}{$opts['log-level']}{else}silent{/if}

# Clash 的 RESTful API
external-controller: '0.0.0.0:9090'

# RESTful API 的口令
secret: '{if array_key_exists("secret",$opts)}{$opts['secret']}{/if}' 

# 您可以将静态网页资源（如 clash-dashboard）放置在一个目录中，clash 将会服务于 `RESTful API/ui`
# 参数应填写配置目录的相对路径或绝对路径。
# external-ui: folder
hosts:
  'www.aloy.asia': 1.1.1.1
  'affcm.com': 1.1.1.1
...
{if $proxymode == 'China'}
{include file='rule/clash/China.yaml'}
{else}
{include file='rule/clash/Global.yaml'}
{/if}
